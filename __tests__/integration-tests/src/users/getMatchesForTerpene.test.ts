...
describe('GET /users/:userId/matches/terpene/:terpene', () => {
  ...
  const request = getGETRequester({
    path: `/users/${userId}/matches/terpene/linalool`,
  ...
  it('calculates matches', async () => {
    scope.get('/stashes/1/terpenes/2/matches?limit=100').reply(200, {
      hasStashData: true,
  ...
  it('queries with limit', async () => {
    scope.get('/stashes/1/terpenes/2/matches?limit=1').reply(200, {
      hasStashData: true,
      matchesByRecommendationLevel: [{ level: 'r4', matches: [matches[0]] }],
    });

    const response = await request({
      path: `/users/${userId}/matches/terpene/linalool?limit=1`,
    });
