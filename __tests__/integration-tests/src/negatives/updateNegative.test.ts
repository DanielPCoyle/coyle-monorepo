...
describe('PATCH /negatives/:id', () => {
  const db = getAdminDb();

  beforeEach(async () => {
    await getAdminDbSeeder(db).withNegatives([
  ...
  it('fails if negative does not exists', async () => {
    const response = await request({ path: '/negatives/3' });
