...
const InksoftEmbed: React.FC<InksoftEmbedProps> = ({
  productId,
  designId,
  styleId,
}) => {
  ...
  useEffect(() => {
    ...
    const launchDesignStudio = () => {
      if (
        !document.getElementById("designStudioIframe") &&
        launchCount.current === 0
      ) {
        console.log(
          `Launching InkSoft Design Studio - Run count: ${launchCount.current}`,
        );
