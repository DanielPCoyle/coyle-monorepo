...
import { products, styles, sizes, sides, suppliers, brands } from '@coyle/database/schema';
...
const ensureSupplierExists = async (data:any) => {
    const db = getDB();
    const existingSupplier = await db.select().from(suppliers).where(eq(suppliers.inksoftId,data.SupplierId)).execute();
    if (existingSupplier.length > 0) {
        return existingSupplier[0].id;
    }
    const [supplier] =  await db.insert(suppliers).values({ inksoftId: data.SupplierId }).returning().execute();
    return supplier.id;    
};
...
const ensureBrandExists = async (data:any) => {
    const db = getDB();
       ...
       const existingBrand = await db.select().from(brands).where(eq(brands.inksoftId,data.ManufacturerId)).execute();
       if (existingBrand.length > 0) {
           return existingBrand[0].id;
       }
        const [brand] =  await db.insert(brands).values({ inksoftId: data.ManufacturerId }).returning().execute();
        return brand.id;
};

const insertProductData = async (data: any) => {
    const db = getDB();
    let [productRow] = await db.select().from(products).where(eq(products.inksoftId, data.ID)).execute();
    if (productRow) {
    ...
    if (!productRow) {
        [productRow] = await db.insert(products).values(product).returning().execute();
    }
    ...
    for (const style of data.Styles) {
        ...
        let [styleRow] = await db.select().from(styles).where(eq(styles.inksoftId, style.ID)).execute();
         if(!styleRow){
            [styleRow] = await db.insert(styles).values(styleData).returning().execute();
         }
