...
const processRecords = async (): Promise<{ count: number; tooBig: number } | undefined> => {
  ...
  let extractedProducts: Product[] = products.Data.map((product: any) => ({
    ...
    Categories: categories.Data.filter((category: any) => category.ItemIds.includes(product.ID)).map((category: any) => category.Name),
  ...
  try {
    ...
    await client.saveObjects({ indexName: 'products_index', objects: extractedProducts as unknown as Record<string, unknown>[] });
