...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  manufacturer = Array.isArray(manufacturer) ? manufacturer : (manufacturer as string).split(',');
  color = Array.isArray(color) ? color : (color as string).split(',');
  category = Array.isArray(category) ? category : (category as string).split(',');
  ...
  try {
    ...
    if (manufacturer.length > 0) {
      filters.push((manufacturer as string[]).map(m => `Manufacturer:"${m}"`).join(' OR '));
    }

    if (color.length > 0) {
      filters.push((color as string[]).map(c => `Styles.Color:"${c}"`).join(' OR '));
    }

    if (category.length > 0) {
      filters.push((category as string[]).map(cat => `Categories:"${cat}"`).join(' OR '));
    }
