...
export default async function handler(req, res) {
  let { color = [], category = [], keyword = '', manufacturer = [] } = req.query;
  ...
  try {
    ...
    // Perform search with keyword and combined filters
    const response = await index.search(keyword, {
      filters: filters.join(' AND '), // Combine filters with AND operator
    });

    res.status(200).json(response); // Send only the hits (filtered products) in the response
  } catch (error) {
