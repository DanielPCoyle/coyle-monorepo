...
describe('addConversation', () => {
    ...
    it('should add a new conversation if it does not exist', async () => {
        ...
        await addConversation({ name: 'John Doe', email: 'john@example.com', conversationKey: 'key123' });
        expect(mockDB.insert).toHaveBeenCalledWith(conversations);
        expect(mockDB.values).toHaveBeenCalledWith({ name: 'John Doe', email: 'john@example.com', conversationKey: 'key123' });
    });
