    ...
    describe("explainAnalyze", () => {
        it("should execute EXPLAIN ANALYZE on the given query", async () => {
            ...
            const mockExecute = vi.fn().mockResolvedValue({
                rows: [{ "QUERY PLAN": "Seq Scan on users  (cost=0.00..1.00 rows=1 width=1)" }],
            });
            const db = getDB();
            db.execute = mockExecute;

            const result = await explainAnalyze(mockQuery as any);
            expect(mockExecute).toHaveBeenCalledWith(sql`EXPLAIN ANALYZE ${mockQuery.getSQL()}`);
